...
```{r eval=T}
taxonomizr_db <- "/Users/tanesc/Documents/DBs/taxonomizr/20211229_taxonomizr/accessionTaxa.sql"
```

```{r}
## read in full length alignment results

### AJ288899 (extra) and MN602556 (missin) have messed up annotations
adf_LTP <- read.delim("/Users/tanesc/Documents/DBs/LTP/LTP_09_2021/LTP_09_2021.csv", header=F, stringsAsFactors = F) %>%
  separate(V4, into=c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus"), sep=";") %>%
...
```{r}
data_dir <- "/Users/tanesc/Documents/unassigner/unassigner_figures/strain_comparison_data"
```
...
assembly_summary <- read_delim("20220224_assembly_summary_refseq.txt", skip = 1, na = c("NA", "na", ""), show_col_types = FALSE)

#assembly_summary <- read_delim("assembly_summary_genbank.txt", skip = 1, na = c("NA", "na", ""), show_col_types = FALSE)

taxa_of_interest <- assembly_summary %>%
  group_by(species_taxid) %>%
  summarize(num_species = n()) %>%
  arrange(-num_species) %>%
  slice(1:20) %>%
  bind_cols(data.frame(getTaxonomy(.$species_taxid,taxonomizr_db))) %>%
  
  
  mutate(species = sub("Staphylococcus aureus", "Staphylococcus aureus subsp. aureus", species)) %>%
  mutate(species = sub("Klebsiella pneumoniae", "Klebsiella pneumoniae subsp. pneumoniae", species)) %>%
  mutate(species = sub("Salmonella enterica", "Salmonella enterica subsp. enterica", species)) %>%
  mutate(species = sub("Enterobacter hormaechei", "Enterobacter hormaechei subsp. hormaechei", species))
...
```{r}
# Using the full-length gene, we identified XXX species that were indistinguishable from at least one other species
unassigner %>%
  filter(mism <= 1) %>%
