...
```{r}
data_dir <- "data/stackebrandt_curves"
```
...
refseq_archaea <- read_tsv(file.path("/Users/tanesc/Documents/unassigner/unassigner_paper_figures/data/stackebrandt_curves/refseq_archaea_assembly_summary.txt"), col_types = assembly_cols, skip=1,
  quote="") %>%
  select(assembly_accession="#assembly_accession", organism_name, species_taxid) %>%
...
```{r}
species_ani_thresholds <- read_csv(file.path(data_dir, "../Fig2/species_ani_thresholds.csv"))
```

```{r}
b <- tibble(fp=list.files(file.path(data_dir, "output/"), full.names = T)) %>%
  rowwise() %>%
...
```{r}
b %>%
  write_csv("unassigner_dataS1_all.csv")
```

```{r}
b <- read_delim("unassigner_dataS1_all.csv")
```
